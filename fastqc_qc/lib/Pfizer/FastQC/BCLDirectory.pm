...
sub buildFromMetaData{

    my $type = shift;
    my $md = shift; # metadata.txt
    my ($name, $path, $suffix) = fileparse($md->filename);
    my $bcl_dir_name;
    ...
    my $bd = new Pfizer::FastQC::BCLDirectory();
    if($bd->selectByName($bcl_dir_name)){
 	print "Found BCL dir with same name \n";
	my $id = $bd->id;	
	my $status = $bd->status;

	print "bcl_dir_id is $id\n";
	print "status $status\n";
   }
