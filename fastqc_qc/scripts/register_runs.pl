...
my $metadata_file = join("/", $FQ_BCL_ROOT_DIR, $target_bcl_dir, $METADATA_FILE);

#print "sample -> $sample_csv\n";
print "metadata file -> $metadata_file\n";

unless (-e $metadata_file) {
    warn "The meta data file '$metadata_file' does not exist";
    exit(-1);
}

my @metadata_files = (new Pfizer::FastQC::Metadata($metadata_file));
unless (@metadata_files) {
    exit(1);
}

register_core(@metadata_files);
